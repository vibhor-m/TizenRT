...
bool 	g_bk_csi_report_timre_running = false;
...
static int bk_wifi_csi_get(uint32_t buf_len, uint8_t *csi_buf, uint32_t *len)
{
	...
	if((g_bk_csi_rept_count > 0) && (g_bk_csi_report_timre_running == false))
	{
		rtos_start_timer(&g_bk_drv->csi_report_timer);
		g_bk_csi_report_timre_running = true;
	}
